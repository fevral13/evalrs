...
impl CacheBackend for LocalMemoryCacheBackend {
    fn set(self: &mut LocalMemoryCacheBackend, key: &str, value: &str) {
        self.cache.put(key.to_string(), value.to_string());
    }

    fn get(self: &mut LocalMemoryCacheBackend, key: &str) -> Option<&String> {
        self.cache.get(&key.to_string()) // fixme: as_ref() worked instead of &...to_string()
    }
}
...
mod test {
    ...
    fn test_cache(){
        ...
        assert_eq!(backend.get(&String::from("key1")).unwrap(), &String::from("value1"));
